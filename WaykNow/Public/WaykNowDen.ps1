...
function Register-WaykNowMachine(){
    ...
    $DenIntermediateCa = "$DenGlobalPath/intermediate_ca.pem"
    $DenRootCa = "$DenGlobalPath/root_ca.pem"
    ...
    $writer.WritePrivateKey($rsa_key);
    ...
    # Check the subject and the Issuer are the expected
    if(!(($leaf_cert.Subject -eq "$subject") -AND ($leaf_cert.Issuer -eq $intermediateAuthority))){
        ...
        throw "Incorrect Sign CRS"
    }

    #11 loader la chaine et valider le leaf certificate en fonction de la chaine + le nom contenu dans le certificat
    #Test With Open SSL: openssl verify -CAfile RootCert.pem -untrusted Intermediate.pem UserCert.pem
    $ekuArray = "$DenIntermediateCa", "$DenCertificatePath"
    Test-Certificate -Cert $leaf_cert -AllowUntrustedRoot -EKU $ekuArray
}
...
Export-ModuleMember -Function Get-WaykNowDen, Connect-WaykNowDen, Disconnect-WaykNowDen, Get-WaykNowDenMachine, Register-WaykNowMachine, Unregister-WaykNowMachine, Get-WaykNowDenRegistration