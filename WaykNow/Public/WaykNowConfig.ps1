...
function Set-WaykNowConfig
{
    ...
    param(
        ...
        # Check for new version on application start
        [bool] $VersionCheck
    )
    ...
    if ($CrashReporterAutoUpload) {
        $json = Set-JsonValue $json 'CrashReporterAutoUpload' $CrashReporterAutoUpload
    }

    if ($VersionCheck) {
        $json = Set-JsonValue $json 'VersionCheck' $VersionCheck
    }
...
function Get-WaykNowConfigInternal
{
    [CmdletBinding()]
...
function Get-WaykNowConfig
{
    ...
    $WaykNowConfigObject.CrashReporterAutoUpload = (Get-NowOptionBool 'CrashReporterAutoUpload' $GlobalServiceAvailable $true $true $LocalJson $GlobalJson)
    $WaykNowConfigObject.VersionCheck = (Get-NowOptionBool 'VersionCheck' $GlobalServiceAvailable $true $true $LocalJson $GlobalJson)
