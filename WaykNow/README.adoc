...
==== Windows Settings
With Windows you can set the global settings or the local settings, by default the local setting is used, if you want to use the global settings in your command add this parameter:

[source,sh]
----
PS ~/WaykNow-ps> Set-WaykNowConfig -Global $true
----
...
==== -ControlMode
Specifies the Remote Control Mode of WaykNow

- *AllRemoteControlMode*: Both sides are displayed

- *TakeRemoteControlClientOnly*: Only the client side is displayed

- *AllowRemoteControlSeverOnly*: Only the server side is displayed.
...
This option is use to prevent the main application window from showing when Wayk Now starts. It can be quite useful when the application is automatically launched.
...
==== -ElevationPrompt
This option is use to disable the prompt to elevate program permissions, and run Wayk Now without elevated program permissions.
...
Option to enable/disable SRP: When Secure Remote Password is disabled, the password options are disabled as well.
...
==== -AllowSystemAuth
Option to enable/disable SRD: Secure Remote Delegation is the method used for system authentication in the case of unattended remote access. On Windows, remote access is restricted to members of the built-in Administrators or Remote Desktop Users groups.
...
==== -AllowNoPassword
Option to enable/disable PFP: Prompt for Permission authentication requests explicit consent from the remote user without the need for a password.
...
==== -DenEnabled
Enable/disable Wayk Den, he is use to simplified peer-to-peer connectivity with a 6-digit ID.
