...
The Wayk Now executable path (referred to as 'wayk-now' in CLI commands) can be detected by the `Get-WaykNowCommand`:
...
==== VersionCheck
Check for updates when Wayk Now is launched
...
[source,sh]
----
PS ~/WaykNow-ps> Set-WaykNowConfig -VersionCheck $true
----
